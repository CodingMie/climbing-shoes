...
- **鞋款目录**：结构化的鞋款信息（品牌、型号、变体、价格、使用场景、硬度、宽度楦型、定位等级、下压程度、闭合方式等），由用户提交、管理员审核后上架，支持按属性筛选与搜索。
...
8. As a 访客, I want to 按关键词搜索鞋款（型号/品牌名）, so that 我可以直达特定鞋款。
9. As a 访客, I want to 在鞋款详情页看到完整参数与图片, so that 我可以了解这双鞋的规格。
10. As a 访客, I want to 在鞋款详情页看到七个维度的平均评分, so that 我可以快速判断口碑。
11. As a 访客, I want to 看到尺码建议分布（相对日常码的偏移占比）, so that 我可以选对尺码。
12. As a 访客, I want to 看到合身度反馈分布（各部位各选项的占比）, so that 我可以预判这双鞋在我脚上可能的合身问题。
13. As a 访客, I want to 阅读某双鞋的全部测评列表, so that 我可以了解真实试穿体验。
14. As a 访客, I want to 在每条测评旁看到作者的脚型摘要（脚宽、足弓、脚背、脚后跟、拇外翻等）, so that 我可以判断这条测评对我是否有参考价值。
15. As a 访客, I want to 在测评详情页看到完整测评内容与作者脚型档案, so that 我可以深入了解细节。
16. As a 访客, I want to 在首页看到热门鞋款与最新测评, so that 我可以快速了解社区动态。
...
30. As a 用户, I want to 提交新鞋款（品牌、型号、变体、价格、场景、硬度、宽度、定位、下压程度、闭合方式、鞋面材质）, so that 目录可以持续扩充。
...
**shoes（鞋款）**：
- brand_id、型号 model、变体 variant、价格 price
...
- `/` 首页；`/shoes` 鞋库（筛选 + 搜索）；`/shoes/[id]` 鞋详情（参数 + 聚合 + 测评列表）；`/shoes/submit` 提交鞋款
...
- 术语表（后续 /domain-modeling 可固化进 CONTEXT.md）：鞋款（shoe model）、变体（variant）、测评（review）、脚型档案（foot profile）、合身度反馈（fit feedback）、尺码偏移（size delta，相对日常码）、审核（moderation）。
